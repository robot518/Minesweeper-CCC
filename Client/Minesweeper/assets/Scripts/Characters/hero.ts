...
import { _decorator, Component, Node, Vec3, Vec2, Animation, lerp, AnimationClip, AnimationState, animation, AnimationComponent, RigidBody, ColliderComponent, ICollisionEvent, BoxCollider, sys, EventKeyboard, macro, KeyCode, input, Input } from 'cc';
import InputDistance from '../utils/input';
...
export class Hero extends Component {
    ...
    update (deltaTime: number) {
        ...
        if (InputDistance.key.left) {
            this.targetRotation += 180 * deltaTime;
        }
        else if (InputDistance.key.right) {
            this.targetRotation -= 180 * deltaTime;
        }
        else if (this.joyStick) {
        ...
        if (InputDistance.key.up || (this.joyStick && this.joyStick.magnitude > 0)) {
        ...
        if (InputDistance.key.space || (this.joyStick && this.joyStick.jump)) {
            if (!this.jumping) {
                this.jumping = true;
